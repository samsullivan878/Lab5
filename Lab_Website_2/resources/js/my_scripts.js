		...
		for(col_counter = 2; col_counter < table.rows[row_counter].cells.length; col_counter++)
			{
				if (col_counter==2){ // home score
					...
					if (cell_value > 0){ // home team won
						home_tally++;
						table.rows[row_counter].cells[col_counter].innerHTML = "Home";
					}
					else{
						away_tally++
						table.rows[row_counter].cells[col_counter].innerHTML = "Away";//Update the actual html of the cell
					}
				}
			}
	}

	document.getElementById("wins").innerHTML = home_tally;
	document.getElementById("losses").innerHTML = away_tally;
}
/*
...
function loadPlayersPage(){
	var src = document.getElementById("player_selector");
	for (i = 0; i<4; i++){
		var newTag = document.createElement('a');
		newTag.setAttribute('href', '#');
		newTag.setAttribute('onclick',"switchPlayers()");
		newTag.innerText = players[i].name;
		src.appendChild(newTag);
	}
}
